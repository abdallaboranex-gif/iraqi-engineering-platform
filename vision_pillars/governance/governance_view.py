...
def render_governance_view():
    ...
    تضمن إخفاء وحجب فحوصات الأبنية العالية حياً عند اختيار الدور السكنية لمنع التحايل.
    """
    ...
    with col_f4:
        req_type = st.selectbox("نوع الطلب / المعاملة", ["بناء جديد", "إضافة طابق", "مشاريع استثمارية كبرى", "ترميم وتعديل"], key="gov_req_type_v5")
    with col_f3:
        total_area = st.number_input("مساحة الأرض الكلية (m²)", min_value=50, max_value=50000, value=200, step=10, key="gov_total_area_v5")
    with col_f2:
        total_floors = st.number_input("عدد الطوابق المقترحة", min_value=1, max_value=60, value=2, step=1, key="gov_total_floors_v5")
    with col_f1:
        has_basement_floor = st.selectbox("هل يتضمن المخطط طابق سرداب؟", ["لا", "نعم"], key="gov_has_basement_floor_v5")

    # احتساب الفئة برمجياً حياً بموجب جدولك
    ...
    # 🏢 الطابق الثاني: انبثاق الحقول التفاعلية الحية بناءً على الشروط الصارمة لجداول الإكسل
    st.markdown('<div class="gov-panel-box">', unsafe_allow_html=True)
    ...
    # المجموعة 1: الفحوصات الأساسية المشتركة (تظهر لجميع الفئات دائماً)
    with gc2:
        ...
        soil_validity = st.selectbox("صلاحية واعتمادية تقرير التربة (Soil_Report_Validity)", ["معتمد ومجاز ومصادق", "غير مصادق / تحت المراجعة"], key="f_soil_validity_v5")
        bh_count = st.number_input("عدد الحفر الاستكشافية المنفذة (Boreholes_Count)", min_value=0, max_value=20, value=2, key="f_bh_count_v5")
        
        # انبثاق الحقل المخصص للعمق بناءً على وزن البناية (طابق ضحل أو برج ثقيل)
        if active_route == "heavy":
            bh_depth = st.number_input("عمق الحفرة الاختبارية للأبراج العالية (Borehole_Depth_Heavy) - المطلوب ≥ 15م", min_value=0.0, max_value=100.0, value=15.0, key="f_bh_depth_heavy_v5")
        else:
            bh_depth = st.number_input("عمق الحفرة الاختبارية للأبنية الخفيفة (Borehole_Depth_Shallow) - المطلوب ≥ 6م", min_value=0.0, max_value=50.0, value=6.0, key="f_bh_depth_shallow_v5")
            
        bearing_capacity = st.number_input("قدرة تحمل التربة التصميمية المسموحة (Soil_Bearing_Capacity) - kN/m²", min_value=0.0, max_value=1000.0, value=150.0, key="f_bearing_v5")
        report_age = st.number_input("عمر التقرير الجيوتقني الحالي بالأشهر (Soil_Report_Age) - المطلوب ≤ 24 شهر", min_value=0, max_value=120, value=6, key="f_report_age_v5")
    ...
    with gc1:
        if active_route in ["medium", "heavy"]:
            ...
            sulphate_so3 = st.number_input("محتوى الكبريتات الثلاثية الذائبة في التربة (Soil_Sulphate_Content_SO3) - المطلوب ≤ 5%", min_value=0.0, max_value=100.0, value=2.1, key="f_sulphate_v5")
            chloride_content = st.number_input("نسبة أيونات الكلوريدات الذائبة (Soil_Chloride_Content) - المطلوب ≤ 0.1%", min_value=0.00, max_value=10.00, value=0.04, step=0.01, key="f_chloride_v5")
            organic_content = st.number_input("محتوى المواد العضوية والجذور (Soil_Organic_Content) - المطلوب ≤ 2%", min_value=0.0, max_value=100.0, value=0.5, key="f_organic_v5")
            compaction_degree = st.number_input("درجة الحدل ورص التربة الميداني (Soil_Compaction_Degree) - المطلوب ≥ 95%", min_value=0.0, max_value=100.0, value=96.5, key="f_compaction_v5")
        else:
    ...
    # المجموعة 3: الفحوصات السيادية الكبرى للأبراج والسرداب فقط (تختفي كلياً لغيرها)
    if active_route == "heavy":
        ...
        g1, g2 = st.columns(2)
        with g2:
            gpr_scan = st.selectbox("نتائج مسح الرادار الأرضي الاختراقي للكهوف (Soil_GPR_Void_Scan)", ["خالٍ من الفجوات والتكهفات الحرجة", "تم رصد فجوات وتجاويف تحت سطحية غير معالجة"], key="f_gpr_v5")
            gypsum_content = st.number_input("نسبة محتوى الجبس الكلية (Soil_Gypsum_Content) - الحد الأعلى الآمن 10.75%", min_value=0.0, max_value=100.0, value=4.2, key="f_gypsum_v5")
        with g1:
            if has_basement_floor == "نعم":
                water_table = st.number_input("منسوب المياه الجوفية المستقر تحت السطح (Water_Table_Depth) - بالمتر", min_value=0.0, max_value=50.0, value=3.5, key="f_water_v5")
            else:
    ...
    if st.button("💳 تأكيد الدفع وتدقيق المعاملة هندسياً للمطابقة الآلية", key="gov_btn_pay_and_audit_v5", use_container_width=True):
        st.success("✅ تم استقطاع الأجور بنجاح! جاري معايرة القراءات رقمياً وتشريعياً مع جداول الأكواد الفيدرالية...")
        st.info("💡 هنت للفريق القوي: الواجهة التفاعلية أصبحت مقيدة بالشروط الحالية ومقسمة بنظام الفرز 100%!")
