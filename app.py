...
# تشفير صورك الأربعة بالأسماء والصيغ الحقيقية
...
# 4. استدعاء باقي طوابق المنصة والكبائن بعد تأمين الهوية البصرية
...
# 5. حقن شريط التنقل العلوي المحمي بالقمة
try:
    show_navbar_section()
except Exception:
    st.error("⚠️ هنت سيادي: عطل طارئ في منظومة شريط التحكم المركزي.")

# 6. إدارة الذاكرة السحابية للتنقل الذكي بين الواجهات
if "current_page" not in st.session_state:
    st.session_state["current_page"] = "home"

# 7. تقسيم الشاشة: اليمين للمحتوى (3.2) واليسار للمؤشرات (1.0) كالأصل
...
# --- الطابق الأيمن الرئيسي: تجميع الكبائن والمدونات السفلية ---
with main_content:
    ...
    if current_view == "home":
        ...
        # عرض كبائن الرؤى الأربعة متراصة في سطر واحد باستخدام صورك وتأثير التوهج
        st.markdown("### 🏢 كبائن الرؤى الاستراتيجية الكبرى للمنصة")
        col_v1, col_v2, col_v3, col_v4 = st.columns(4)
        
        # كابينة 1: المدن الذكية بمفتاح نهائي معزول
        with col_v1:
            st.markdown(f"""
            ...
            if st.button("استكشف واجهة المدن 🗺️", key="ultimate_go_sc", use_container_width=True):
                st.session_state["current_page"] = "smart_cities"
                st.rerun()

        # كابينة 2: حوكمة المشاريع بمفتاح نهائي معزول
        with col_v2:
            st.markdown(f"""
                <div style="background-color: rgba(7, 22, 21, 0.7); border-radius: 12px; border: 1px solid rgba(197, 160, 89, 0.3); padding: 0px; text-align: center; overflow: hidden; box-shadow: 0 4px 15px rgba(197, 160, 89, 0.25); margin-bottom: 8px;">
            ...
            """, unsafe_allow_html=True)
            if st.button("استكشف واجهة الحوكمة 🔒", key="ultimate_go_gov", use_container_width=True):
                st.session_state["current_page"] = "governance"
                st.rerun()

        # كابينة 3: الأتمتة والذكاء الاصطناعي بمفتاح نهائي معزول
        with col_v3:
            st.markdown(f"""
                <div style="background-color: rgba(7, 22, 21, 0.7); border-radius: 12px; border: 1px solid rgba(197, 160, 89, 0.3); padding: 0px; text-align: center; overflow: hidden; box-shadow: 0 4px 15px rgba(197, 160, 89, 0.25); margin-bottom: 8px;">
            ...
            """, unsafe_allow_html=True)
            if st.button("استكشف واجهة الأتمتة 🧠", key="ultimate_go_auto", use_container_width=True):
                st.session_state["current_page"] = "automation"
                st.rerun()

        # كابينة 4: الاستدامة وكفاءة الطاقة بمفتاح نهائي معزول
        with col_v4:
            st.markdown(f"""
                <div style="background-color: rgba(7, 22, 21, 0.7); border-radius: 12px; border: 1px solid rgba(197, 160, 89, 0.3); padding: 0px; text-align: center; overflow: hidden; box-shadow: 0 4px 15px rgba(197, 160, 89, 0.25); margin-bottom: 8px;">
            ...
            """, unsafe_allow_html=True)
            if st.button("استكشف واجهة الاستدامة ☀️", key="ultimate_go_sustain", use_container_width=True):
    ...
    # --- إدارة تنقل وفتح الواجهات الجديدة والمستقلة بالكامل عند ضغط الأزرار ---
        ...
        if st.button("↩️ العودة للشاشة الرئيسية", key="ultimate_back_sc"): st.session_state["current_page"] = "home"; st.rerun()
        ...
        if st.button("↩️ العودة للشاشة الرئيسية", key="ultimate_back_gov"): st.session_state["current_page"] = "home"; st.rerun()
        ...
        if st.button("↩️ العودة للشاشة الرئيسية", key="ultimate_back_auto"): st.session_state["current_page"] = "home"; st.rerun()
        ...
        if st.button("↩️ العودة للشاشة الرئيسية", key="ultimate_back_sus"): st.session_state["current_page"] = "home"; st.rerun()
        ...
        if st.button("↩️ العودة للشاشة الرئيسية", key="ultimate_back_nav_pages"): st.session_state["current_page"] = "home"; st.rerun()
