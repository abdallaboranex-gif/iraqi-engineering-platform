...
def render_governance_view():
    ...
    with col3:
        gov_province = st.selectbox("1. المحافظة (النطاق الجغرافي للعقار)", ["اختر بنداً...", "بغداد", "نينوى", "البصرة", "صلاح الدين", "الأنبار", "النجف", "كربلاء", "بابل", "القادسية", "المثنى", "ذي قار", "ميسان", "واسط", "ديالى", "كركوك", "أربيل", "السليمانية", "دهوك", "حلبجة"], key="v10_prov_final")
        gov_req_type = st.selectbox("2. نوع الطلب / المعاملة", ["اختر بنداً...", "بناء جديد", "إعادة بناء", "إضافة طابق", "ترميم وتعديل", "مشاريع استثمارية كبرى / أبراج"], key="v10_req_final")
        gov_property_use = st.selectbox("3. استعمال العقار الأساسي", ["اختر بنداً...", "سكني", "تجاري", "خدمي", "صناعي", "مجمعات ومستشفيات / أبنية عامة"], key="v10_use_final")
    with col2:
        gov_area = st.number_input("4. مساحة العقار الكلية (m²)", min_value=0, max_value=1000000, value=0, step=10, key="v10_area_final")
        gov_length = st.number_input("5. أبعاد العقار - الطول (m)", min_value=0.0, max_value=2000.0, value=0.0, step=0.5, key="v10_length_final")
        gov_width = st.number_input("6. أبعاد العقار - العرض/الواجهة (m)", min_value=0.0, max_value=1000.0, value=0.0, step=0.5, key="v10_width_final")
    with col1:
        gov_street_w = st.number_input("7. عرض الشارع المقابل للعقار (m)", min_value=0, max_value=200, value=0, key="v10_street_final")
        gov_floors = st.number_input("8. عدد الطوابق المقترحة / ارتفاع المبنى", min_value=0, max_value=100, value=0, step=1, key="v10_floors_final")
        gov_has_basement = st.selectbox("9. طابق السرداب (Basement)", ["اختر بنداً...", "غير موجود", "موجود"], key="v10_basement_final")

    st.markdown("<hr style='border-color: rgba(197, 160, 89, 0.1); margin: 10px 0;'>", unsafe_allow_html=True)
    gov_id_text = st.text_input("📝 هوية العقار الرسمية (رقم العقار / القطعة والمقاطعة)", placeholder="مثال: 4/1250 مقاطعة 21 داودي", key="v10_id_text_final")
    ...
    if active_route == "locked":
        ...
        with gc2:
            ...
            if active_route == "heavy":
                bh_depth = st.number_input("عمق الحفرة الاختبارية المنفذة للأبراج العالية (Borehole_Depth_Heavy) - متر", min_value=0.0, max_value=150.0, value=0.0, key="f_bdh_v10")
            else:
                bh_depth = st.number_input("عمق الحفرة الاختبارية المنفذة للأبنية الخفيفة (Borehole_Depth_Shallow) - متر", min_value=0.0, max_value=50.0, value=0.0, key="f_bds_v10")
                
            bearing_capacity = st.number_input("قدرة تحمل التربة التصميمية المسموحة المعتمدة (Soil_Bearing_Capacity) - kN/m²", min_value=0.0, max_value=2000.0, value=0.0, key="f_bc_cap_v10")
        ...
        if active_route == "heavy":
            ...
            with g2:
                if gov_province in ["النجف", "الأنبار", "المثنى", "نينوى"]:
                    ...
                    gpr_scan = st.selectbox("نتائج مسح الرادار الأرضي الاختراقي للكهوف والكهوف الجيرية (Soil_GPR_Void_Scan)", ["خالٍ من الفجوات والتكهفات الحرجة", "تم رصد فجوات وتجاويف تحت سطحية غير معالجة"], key="f_gpr_v10")
                else:
                    gpr_scan = "خالٍ من الفجوات والتكهفات الحرجة"
                gypsum_content = st.number_input("نسبة محتوى الجبس الكلية لطبقات التربة السطحية (Soil_Gypsum_Content) - %", min_value=0.0, max_value=100.0, value=0.0, key="f_gyp_v10")
            with g1:
                if gov_has_basement == "موجود":
                    water_table = st.number_input("منسوب المياه الجوفية المستقر تحت السطح (Water_Table_Depth) - بالمتر", min_value=0.0, max_value=100.0, value=0.0, key="f_wt_v10")
                else:
